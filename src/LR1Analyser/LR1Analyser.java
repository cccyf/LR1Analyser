...
public class LR1Analyser {
    ...
    public List<String> Analyse() {
        ...
        while (true) {
            ...
            Action action = ParsingTable.getAction(currentState, currentToken.getTokenType());
            ...
            if (action.getActionType() == ActionType.SHIFT) {
                ...
                String reduce = ParsingTable.getProduction(action.getNumber());
                ...
                if (ParsingTable.belongToGOTOs(symbolStack.peek())){
                    currentState = ParsingTable.getGOTO(currentState,symbolStack.peek());
                    stateStack.push(currentState);
                    states = currentState+" "+states;
                    System.out.println(states+"					"+symbols+"					"+currentToken.getLabel()+"					"+action.toString());
                ...
                reductions.add(ParsingTable.getProduction(0));
                symbolStack.pop();
                stateStack.pop();
                symbols= symbols.substring(symbols.indexOf(" ")+1);
                states = states.substring(states.indexOf(" ")+1);
